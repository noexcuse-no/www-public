...
1. **Guide, don't decide** — Present options and let the user decide. Don't make assumptions about your preferences.
...
1. **Check design documentation** — If the project has a `.design/` directory, read relevant design specs (colors, brand guidelines, layout specs) to understand visual requirements.

2. **Check for text in prompts** — If your prompt contains words, labels, or text elements to appear in the image, **STOP and ask the user**:
   
   > "Your prompt contains text: '[the text]'. For multilingual websites, avoid text in generated illustrations — it creates localization issues and the AI may generate text incorrectly. Options:
   > - Generate without text and add text via CSS/HTML overlay
   > - Use abstract/symbolic imagery instead
   > - Proceed with text anyway (acknowledge limitation)
   > 
   > Which do you prefer?"

3. **Check language requirements** — If the site is Norwegian (like noexcuse.no), ensure prompts describe imagery without Norwegian text unless explicitly requested.

## Example Workflow for Text-Containing Prompts

**Before generating this prompt:**
> "Illustration with text saying 'Ledelse 60:2' and organizational chart"

**Ask the user:**
> "I see you're requesting text in the image. For Norwegian websites with multilingual needs, I recommend against embedded text. Better approaches:
> - Generate illustration without text, add 'Ledelse 60:2' via CSS overlay
> - Use symbolic imagery (org chart without labels)
> - Proceed with text (may have AI generation limitations)
> 
...
| **Reference images** | For image editing: 1-16 images via publicly accessible URL (JPEG/PNG/WebP, <=50MB each). **Local files NOT supported** — must use HTTPS URLs. Ask for URLs if editing mode is detected. | Conditional |
...
## Important: One Image Per Prompt

**When you need multiple distinct images, generate them in SEPARATE requests.** If you combine instructions for several discrete images in the same prompt, the model will generate all elements in a single composition rather than as independent images.

**Correct approach:**
- Prompt 1: "Geometric hexagon pattern representing structure frame, blue color, minimalist Scandinavian style, fading transparent at bottom"
- Prompt 2: "Abstract circle pattern representing human frame, green color, minimalist Scandinavian style, fading transparent at bottom"
- And so on for each distinct image

**Incorrect approach:**
- "Geometric hexagon for structure (blue), circle for human (green), diamond for political (amber), star for symbol (purple) all in one image"
...
| Image processing error | "The reference image URL is inaccessible. Use a publicly accessible HTTPS URL (e.g., your deployed website). Local file paths are not supported." |
...
- **Image editing**: Provide 1-16 reference images via publicly accessible HTTPS URL and describe the edits. **Local file paths are NOT supported.**
