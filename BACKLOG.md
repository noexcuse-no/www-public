...
| Q1 | **Quiz — data model + question pool** — Create 15+ questions with 6+ answers each, mapped to Bolman & Deal's four frames. Norwegian Bokmål, brand voice. Output: `_data/quiz/questions.json`. See `.specs/quiz/README.md`. | Planned | Q3 |
...
| Q3 | **Quiz — JS engine** — Create `assets/scripts/quiz.js`: modal controller, question flow, scoring, results, sessionStorage, share buttons. No dependencies. See `.specs/quiz/README.md`. | Planned | — |
| Q4 | **Quiz — CSS** — Create `assets/css/components/quiz.css`: modal overlay, progress bar, answer cards, result infographic, score bars. Dark mode via CSS variables. See `.design/quiz.md`. | Planned | Q3 |
| Q5 | **Quiz — page integration** — Add quiz CTA to homepage and relevant article pages. Wire JS/CSS includes in `_includes/scripts.html` and `_includes/styles.html`. Add `quiz-modal.html` include to layouts. See `.specs/quiz/README.md`. | Planned | Q3, Q4 |
