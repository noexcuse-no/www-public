...
| R37 | **Fix dark mode on profile pages** — Profile content invisible on `/dagfinn/` and unreadable on `/om-oss/` in dark mode. Add missing `.profile-person-title` dark override, verify profile card backgrounds. See `.omo/plans/ui-ux-fixes-r27.md` Branch 1. | Done | — |
| R38 | **Fix horizontal scroll on mobile** — `/personvern/`, `/pavirkning/`, `/grc/` overflow at ≤390px. Root cause: tables and images in lists exceed viewport. Add `overflow-x: auto` to tables, ensure images wrap. See `.omo/plans/ui-ux-fixes-r27.md` Branch 1. | Done | — |
| R39 | **Fix `/identitet/` horizontal scroll at desktop** — 1920px/1440px dark mode overflow. Same root cause as R38 — images in list items. See `.omo/plans/ui-ux-fixes-r27.md` Branch 1. | Done | — |
| R40 | **Fix hero height regression at 1366×768** — 23 pages affected. Header actual height is 97px not 85px, plus 39px gap pushes hero below fold. Update `--header-height` or hero calc. See `.omo/plans/ui-ux-fixes-r27.md` Branch 2. | Planned | — |
| R41 | **Fix broken images on `/grc/` and `/pavirkning/`** — Markdown references use `å` but filesystem uses ASCII `a`. Update image paths in `_pages/påvirkning.md` and `_pages/grc.md`. See `.omo/plans/ui-ux-fixes-r27.md` Branch 2. | Planned | — |
| R42 | **Fix tag cloud slugify bug** — `_includes/tag-cloud.html:27` applies `slugify` to full path, converting `/emne/tag` to `/emne-tag`. Fix Liquid filter grouping. See `.omo/plans/ui-ux-fixes-r27.md` Branch 3. | Planned | — |
| R43 | **Create missing `_tags/` files or filter tag cloud** — 10 of 14 tags lack pages. Filter cloud to existing tags only. See `.omo/plans/ui-ux-fixes-r27.md` Branch 3. | Planned | — |
| R44 | **Fix `animations.js` initialization** — Back-to-top and scroll indicator broken. Verify script loading, check for CSP blocks, add robust fallback. See `.omo/plans/ui-ux-fixes-r27.md` Branch 4. | Planned | — |
| R45 | **Fix mobile TOC modal at 1024px** — Modal stuck visible at 1024px-1199px range. Ensure `hidden` attribute and media query alignment. See `.omo/plans/ui-ux-fixes-r27.md` Branch 4. | Planned | — |
| R46 | **Fix GRC card grid single-column bug** — `.card-grid--grc` defined inside `@media (max-width: 900px)` so above 900px defaults to 1 column. Move outside media query. See `.omo/plans/ui-ux-fixes-r27.md` Branch 5. | Planned | — |
| R47 | **Increase touch targets to ≥44px** — Tag pills ~30px, footer links ~16px. Increase padding. See `.omo/plans/ui-ux-fixes-r27.md` Branch 5. | Planned | — |
| R48 | **Fix homepage dark mode** — Navbar and hero un-themed on `/` in dark mode. Verify dark overrides. See `.omo/plans/ui-ux-fixes-r27.md` Branch 5. | Planned | — |
...
| R50 | **Update CSS breakpoint documentation** — `.design/css-architecture.md:160` claims 1024px breakpoint but actual breakpoints are 599px, 768px, 900px, 1199px. See `.omo/plans/ui-ux-fixes-r27.md` Branch 6. | Planned | — |
| R51 | **Remove stale dark mode test and dead toggle CSS** — `tests/dark-mode.test.js` references non-existent `#dark-mode-toggle`. `utilities.css` has dead toggle styles. Remove both. See `.omo/plans/ui-ux-fixes-r27.md` Branch 6. | Planned | — |
| R52 | **Add custom 404 page and noscript fallbacks** — Create `_pages/404.md`. Add `<noscript>` blocks for nav, TOC, questions. See `.omo/plans/ui-ux-fixes-r27.md` Branch 6. | Planned | — |
| R53 | **Remove personal data from `_pages/dagfinn.md`** — Phone and email in public repo. Remove from frontmatter, update profile rendering. See `.omo/plans/ui-ux-fixes-r27.md` Branch 6. | Planned | — |
| R54 | **Add tag pages to sitemap.xml** — 12 tag pages (`/emne/*`) missing from sitemap. Update Jekyll sitemap config. See `.omo/plans/ui-ux-fixes-r27.md` Branch 3. | Planned | — |
| R55 | **Fix URL mismatches** — `/om-metode/` → `/metode/`, `/påvirkning/` → `/pavirkning/`. Update internal links and IA map. See `.omo/plans/ui-ux-fixes-r27.md` Branch 6. | Planned | — |
