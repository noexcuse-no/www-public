...
### Changed
- **Booking URL standardization (samtale v3.1)**: `/samtale/` booking CTA switched from the v3 mailto fallback to the canonical site-wide Bookings URL (`outlook.office.com/book/ledelse@noexcuse.no/?ismsaljsauthenabled`) per owner confirmation that the 20-min avklaring service is live; supporting copy adjusted ("Velg et tidspunkt som passer i kalenderen."). `_layouts/tag.html` links gained the missing `?ismsaljsauthenabled` param for consistency. Booking-click analytics selector in `auto-events.js` now matches `a[href*="outlook.office.com/book/"]` (mailto selector removed). Deferred `booking-fallback-form.html` stays unreferenced; `_data/forms.yml` placeholders untouched. Note: lint/tests/CI still not executable in this environment — run `npm run lint && npm test` before PR.
- **Copy discipline revision (site-wide)**: Terminology and claims pass across content pages, includes, data, and scripts. Product self-description standardized on "kunnskapsbasert orientering for ledergrupper" (replacing the old diagnostic-framing term) in the reflection-landing template on all step/article pages and `review-questions.js`; "diagnostiske spørsmål" → "strukturerte spørsmål"; measurement verbs bounded ("måler" → "undersøker"/"synliggjør"; modenhet/baseline/beste praksis claims reframed or removed unless tied to a defined model); research references framed interpretively (Logan, Hubbard); synthetic case studies on /perspektiv/ relabeled "Illustrativt eksempel"; Southwest Airlines example removed from /mennesker/; AI-transparency wording professionalized ("Utforsk med din egen KI") across pages, `_includes/ai-reflection-cta.html`, `_includes/questions.html`, and `_layouts/article.html` noscript blocks; "Hva skjer med data fra gjennomføringen?" section added to /metode/ (plus NESH link-text cleanup); /samtale/ no longer hides the site AI banner; deprecated `scripts/clean-ai-ctas.py` disabled with deprecation stub. Fact fix: organization number corrected to 935 794 420 in /personvern/. /om-oss/ national-character and quality-signal claims rewritten to bounded "Norsk utgangspunkt" framing, duplicate method paragraph trimmed, "Vi diagnostiserer" section reframed as "Vi undersøker". Language repairs: arbetet→arbeidet, Hubbarts→Hubbard, mærkes→merkes, Manglande→Manglende, bekrefteise→bekreftelse, Et åpne→Et åpent, Metodikk vår→Metodikken vår, testet→utviklet gjennom bruk (ledelse-60-2). Second register pass (Norwegian prose quality): defensive "X — ikke Y" contrasts and afterthought tails removed across homepage, /ledelse-60-2/ and /om-oss/ ("på overflaten", "ikke generisk teoristoff", "Poenget er ikke å skåre", "— ikke det vi ønsker skulle skje" etc.); English leakage replaced (lock-in→binding, survey/scoreboard→spørreundersøkelse/poengmåling, scoring/ranking→poengsetting/rangering, løpende compliance→løpende etterlevelsesarbeid, governance/compliance (GRC)→virksomhetsstyring/etterlevelse); consultancy slogans rewritten as plain claims ("skjerpe mandatet", "Kunden eier resultatet — vi leverer, dere bestemmer", "Uavhengig diagnose — ingen binding"); `_includes/cta-panel.html` reassurance bullets and `newsletter` lead cleaned; /ledelse-60-2/ title uses colon separator; related-perspectives labels use colons and related-topics padding removed; reflection footer rewritten ("Du startet denne refleksjonen fra noexcuse.no"); KI reflection opening question reworded without forced binary. Register rows targeting the live /om-store-sprakmodeller/ seed page are full drift against the older long-form version on this branch — page untouched. Tag page `_tags/ledelsesdiagnostikk.md`, its `rights.json` entry, and CHANGELOG history intentionally untouched (URL/tag stability). Note: lint/tests/CI not executable in this session — run `npm run lint && npm test` before PR.

- **Homepage repositioning v2**: Front page rebuilt to a fixed 12-block editorial sequence — hero (two-column, illustration beside text, exact H1/CTA copy), recognition (4 signal cards, 2×2, no mobile carousel), outcomes (3 numbered items with accent dividers), diagnose-before-tiltak (full-width surface, 40/60 split), process (`#slik-fungerer-det`, 3 light steps), fit/not-fit (direct `.fit-column.fit--not` × selector), price (single strong card, `kr 14 850,-` from `_data/commercial.yml`), four perspectives (`#fire-perspektiver`, compact text links), final CTA with Dagfinn — and nothing substantial after the final CTA (proof/newsletter/partners/profile section removed from the page). `home.css` is now the single owner of homepage layout with scoped tokens (`--content-reading: 68ch`, `--content-wide: 1180px` on `.home-content` only; shared 800px token untouched for article pages). Recognition styles removed from `commercial.css` (incl. the mobile snap-scroll). AI strip rewritten to the exact professional disclosure with «Slik bruker vi KI» → `/om-store-sprakmodeller/` and the unprofessional source comment removed (protected verbatim moved to `banner-verbatim.test.js`'s new assertions). Footer gains «Om KI» and renders `© 2025–<build year>` via Liquid (single year when equal). Themed `--error-color`/`--success-color` raw pairs fix ✓/× marker contrast in dark mode (× 6.7:1, ✓ 7.2:1). Profile component (still used on the product page) repaired for accessibility: dialog opened by a semantic button, `role="dialog"` + `aria-modal` + `aria-labelledby`, focus trap, Escape/backdrop close, focus return; inline script moved to `assets/scripts/profiles.js`. Verified at 390/768/1024/1440 in both themes: no horizontal overflow, spec column counts, 96/72/56px section rhythm, 32px/20px container padding, full-width hero CTAs <480px.

### Fixed
- **Lint toolchain repair (RA3)**: `npm run lint` green again after dependabot #261 drift broke it. eslint 6→10: migrated `.eslintrc.yml` to `eslint.config.js` flat config (same rules, `assets/scripts/**` scope) and applied the configured formatting to scripts (indent 4, single quotes). htmlhint 1.9.2: disabled `id-class-value`/`class-style` (cannot express canonical BEM `--` modifiers — naming owned by stylelint + css-conventions), `spec-char-escape` (false positives on Liquid comparison operators in templates) and `inline-style-disabled` (documented dynamic-value exceptions); the 4 static inline-style violations it surfaced were fixed properly (`article.html` noscript blocks → `.noscript-note`, `sidebar-home.html` → `.sidebar-tagline`, CSS in `sidebar.css`). Toolchain exact-pinned per repo convention (eslint 10.10.0, htmlhint 1.9.2, stylelint 17.15.0, stylelint-config-standard 40.0.0, vitest 5.0.0, happy-dom 20.14.3) and `vite@7.3.6` + `globals@17.12.0` declared explicitly (required peers no longer bundled). npm normalization also removed a duplicated `scripts` block from `package.json` (duplicate JSON keys were last-wins — runtime behavior unchanged). Full suite: 260/260; all local CI gates green.

### Changed
- **Spec-impl consistency audit (RA1)**: Full reconciliation of every active spec, design doc, and process rule against the implementation. BACKLOG.md pruned to open work and shipped-item statuses reconciled (V1–V6, MR1, R36/R37, AL1, BU1, RI1 → Done with evidence refs); CHANGELOG consolidated ([Unreleased] first, versions strictly descending, duplicates dropped). Surgical doc corrections: AGENTS.md duplicate linting row + dead collection matchers removed, `.omo/rules/frontmatter.md`/`brand-voice.md`/`content-management.md` repointed from dead `_profiles/*`/`_products/*` globs to current paths, seo/social-previews/semantic-metadata spec statuses aligned with shipped state, perspektiv/triader/makt spec File sections repointed to post-rename paths. i18n spec annotated as the one verified planned-not-implemented item (FF2 Blocked). Duplicate `_pages/om_store_sprakmodeller.md` (same permalink as the canonical page, no provenance block) deleted. Registered 7 confirmed-orphan templates (2 layouts, 5 includes) as fix spec `.specs/cross-cutting/stale-template-cleanup/README.md` (RA2). Central report: `.specs/reconciliation-audit/README.md`. Owner-decision list: brand-trait model (RA-E002), quiz block (RA-E003), orphan deletion approval (RA2).
- **Test/tooling hardening (RA1)**: `tests/setup.js` guards the read-only `global.navigator` assignment; `scan-sensitivity.mjs` excludes `.playwright-mcp/` artifacts; `sanitize-metadata.sh` excludes exiftool file-path headers from local-path detection; `consistency-sources.json` allowlists `LicenseRef-EU-AI-Basic-Icon`; `check-stale-references.mjs` allowlists `_config.yml` and the reconciliation-audit spec; slowness-prone scanner tests given explicit timeouts; `audit-deps.mjs` skips known-brand names in typosquat detection and adds `yaml` to the brand list. Full suite: 260/260 green.
...
### Added
- **Newsletter signup (G4 F4a)**: Email capture form on article pages and homepage with GDPR consent checkbox. Submits to configurable external endpoint — no repo storage. New files: `_includes/newsletter-signup.html`, `assets/css/components/newsletter.css`, `assets/scripts/newsletter.js`. Wired into `_layouts/article.html` and `_layouts/home.html`. Includes email validation, loading states, success/error feedback, and accessibility attributes (aria-live, aria-describedby).
- **Security policy & security.txt renewal (T13)**: Added `.github/SECURITY.md` with vulnerability reporting policy pointing to established contact `mailto:firmapost@noexcuse.no`. Renewed `/.well-known/security.txt` with fresh `Expires: 2027-09-10`, added `Preferred-Languages: en, no` and `Policy` URL. Updated `.design/information-architecture.md` with special resources section listing `/.well-known/security.txt`.
- **Local supply-chain controls (T14)**: Added local npm scripts for dependency audit (`audit:deps`) and supply-chain security checks (`check:supply-chain`) as GitHub Pages native build has no Actions. `scripts/audit-deps.mjs` runs `npm audit`, outdated check, slopsquat detection, and lockfile verification. `scripts/check-supply-chain.mjs` runs security pattern scans (eval, Function, innerHTML, secrets) and optional Semgrep. Added `.github/dependabot.yml` (optional, for GitHub native Dependabot PRs). Updated `.omo/rules/linting.md` with new commands and semgrep tool.
- **Post-build publication audit (T15)**: Added `scripts/audit-site-publication.mjs` to scan `_site/` for internal material leaks (.design/, .specs/, .research/, .omo/, .github/, .git/, node_modules/, test fixtures, .DS_Store, source maps, internal paths, secrets). Allowlist for approved specials (.well-known/*, assets/**, sitemap.xml, robots.txt, site.webmanifest, favicons). `scripts/site-audit-allowlist.json` defines allowlist. Updated `_config.yml` with exclude patterns to prevent internal dirs from being copied to `_site/`. Added `npm run audit:site` script and tests.
- **Local CI runner orchestration (T16)**: Added `scripts/run-ci.mjs` (`npm run ci`) running all checks sequentially (lint, test, sensitivity, secrets, audit:deps, check:supply-chain, audit:site, check:stale, check:docs, check:consistency). Each check runs independently, reports pass/fail, exit code = failure count. No GitHub Actions — runs locally per GitHub Pages native build. Updated `.omo/rules/linting.md` with `ci` command docs.
- **Stale-reference integrity (T17)**: Added `scripts/check-stale-references.mjs` (`npm run check:stale`) scanning active docs for references to deleted/archived paths (.research/, .design/archive/, .specs/archive/, bootstrap docs). Allowlists CHANGELOG.md, config files, test files. `scripts/stale-paths.json` defines 60+ deleted paths.
- **Document structure validation (T18)**: Added `scripts/check-doc-structure.mjs` (`npm run check:docs`) flagging docs in active .design/ and .specs/ with flagged frontmatter status (archived, superseded, obsolete, done). Uses `status` frontmatter field; absent status allowed; CHANGELOG.md and archive dirs allowlisted. `scripts/doc-conventions.json` defines conventions.
- **AI/transparency policy page (T19)**: Added `_pages/om_store_sprakmodeller.md` placeholder for ai-act sibling "Om KI" page at `/om-store-sprakmodeller/`. Documents six principles: KI as tool not decision-maker, mixed authorship categories, editorial responsibility retained, KI doesn't determine copyright/licensing, per-resource provenance/rights metadata, no internal operational detail public.
- **Why public repo statement (T20)**: Added "Hvorfor dette repositoriet er offentlig" section to README.md with mandated principle: public repo documents current website, implementation, maintained specs; temporary planning/commercial reasoning/obsolete context excluded. Framed as inspectability/accountability. Links to AI/transparency page and ai-transparency.json.
- **Transparency consistency validation (T21)**: Added `scripts/check-transparency-consistency.mjs` (`npm run check:consistency`) validating site-level vs per-resource AI/rights assertions. Detects forbidden blanket assertions ("all content is AI/CC0"), validates canonical sources (.well-known/ai-transparency.json, AI policy page, photography brief, apply-provenance script, site metadata).
- **Cross-cutting provenance/rights tests (T22)**: Added `tests/provenance-rights-validators.test.mjs` (31 tests) covering sensitivity patterns, site audit, stale references, provenance/rights validators, human asset non-inheritance, secret echo prevention, fixture hygiene, and existing scans integration.
### Security
...
- Added AI/rights transparency consistency validation to prevent blanket ALL-AI/ALL-CC0 claims (T21).
...
### Removed
- **Risk-reduction Phase 1 cleanup**: Deleted `.research/` (internal research corpus), `.design/SPEC.md`, `.design/website-gaps-overview.md`, `.design/codebase-integrity.md`, `.design/archive/`, `.specs/archive/` (24 completed specs), and five completed one-time implementation specs (codebase-integrity, ledelse-60-2-ui-ux, production-ui-audit, regression-fixes-phase6, profile-card-test-fixes). Their records live in git history.
 Single-source-of-truth strategy doc `.design/inbound-strategy.md` (commercial thesis, positioning, target segments, I0–I6 visitor intent, purchase routes A/B, price rules, /bestill/ architecture, AI private-reflection model, privacy 3 layers, analytics, design principles, publicity→topic mapping, multilingual principle, product-family table, intentionally-unresolved list). Aligned `.design/` docs (SPEC.md, brand-perception, components, information-architecture, wide-screen-sidebar, website-gaps-overview, grc, photography-brief). Completed 6 V-spec stubs (V1–V6: homepage-repositioning, product-page-offer, navigation-ia-split, buying-situation-pages, article-conversion-rails, founder-credibility). Superseded booking-direct-links + ai-agent-cheat-sheets; updated inbound-sales, cta-frontmatter, ai-disclaimer, privacy, i18n, partners. Added 7 new specs (bestill-booking-page, ai-private-reflection, privacy-communication, analytics-events, foredrag-media, vanity-qr-attribution, product-signature). BACKLOG dispositions: FF2 note corrected, Q1–Q5 → Blocked, R33 superseded, G4 → Done, V1–V6 wave items added. P0–P2 entries land per-wave as work merges.
- **Color system refactor (CS1, Branch 3)**: Restructured `colors.css` into 3 sections (Constants / Raw pairs / Active defaults). Introduced unsuffixed active variables (`--text-color`, `--box-background`, `--border-color`, `--surface-subtle`, `--surface-hover`, `--shadow-sm`, `--shadow-md`, `--overlay-modal`, `--accent-color`, `--focus-color`, `--carousel-btn-bg`, `--hero-text`, `--hero-overlay`, `--footer-bg`, `--header-bg`, `--disclaimer-bg`, etc.) that switch mode via the mode stylesheets. Eliminated all 136 `.dark-mode` selectors across 21 CSS files — mode switching is now purely variable-driven. Rewrote `styles-light.css`/`styles-dark.css` as pure `:root {}` variable assignment blocks (zero element selectors). Removed alias/dead variables (`--primary-accent`, `--box-shadow-light`, `--surface-raised-*`, `--close-bg-*`, `--banner-*`, etc.). Collapsed surface variables. Changed dark card background from `#333333` to `#1e2a3a`. Fixed dark-mode button text contrast (WCAG). Updated test suite (76 CSS variable tests).
- **Color system documentation (CS1, Branch 4)**: Rewrote `.design/colors.md` to match the refactored variable architecture (constants / raw pairs / active defaults; removed twin-swap header claim, `--nav-link-color-dark`, `--banner-*`, alias docs; documented new `--frame-*`, `--brand-*`, `--error-color`, `--success-color`, `--focus-color`, `--carousel-btn-*`, `--hero-text`, `--hero-overlay*`). Rewrote `.design/css-architecture.md` theme section for variable-only dark mode (removed `[data-theme]` and "no component re-selection" claims; documented the stylesheet `disabled` attribute toggle mechanism; updated naming convention and design tokens). Fixed `.design/architecture.md` dark-mode-toggle.js description. Added R56–R59 BACKLOG entries.
- **Inbound strategy alignment (W1, master-execution-orchestration)**: Single-source-of-truth strategy doc `.design/inbound-strategy.md` (commercial thesis, positioning, target segments, I0–I6 visitor intent, purchase routes A/B, price rules, /bestill/ architecture, AI private-reflection model, privacy 3 layers, analytics, design principles, publicity→topic mapping, multilingual principle, product-family table, intentionally-unresolved list). Aligned `.design/` docs (SPEC.md, brand-perception, components, information-architecture, wide-screen-sidebar, website-gaps-overview, grc, photography-brief). Completed 6 V-spec stubs (V1–V6: homepage-repositioning, product-page-offer, navigation-ia-split, buying-situation-pages, article-conversion-rails, founder-credibility). Superseded booking-direct-links + ai-agent-cheat-sheets; updated inbound-sales, cta-frontmatter, ai-disclaimer, privacy, i18n, partners. Added 7 new specs (bestill-booking-page, ai-private-reflection, privacy-communication, analytics-events, foredrag-media, vanity-qr-attribution, product-signature). BACKLOG dispositions: FF2 note corrected, Q1–Q5 → Blocked, R33 superseded, G4 → Done, V1–V6 wave items added. P0–P2 entries land per-wave as work merges.
...
- **Jekyll native pages (A5)**: Converted the `_pages` directory from a shadowing `pages` collection to native Jekyll pages loaded via `include: ["_pages"]`. Renamed the frontmatter `name` key to `display_name` in `_pages/ledelse_60-2.md` and `_pages/dagfinn.md` (Jekyll::Page's built-in `name` shadows the key on native pages) and updated 12 template references. Added deterministic `sort: "title"` to three class-filtered lists. Added structural regression test. No page changes address; the built site is byte-identical except article order on `/emne/<tag>/` pages.
...
- **Provenance pipeline (Z2.1–Z2.5)**: End-to-end AI-generated content disclosure system:
  ...
  - **Z2.4 — Image provenance script**: `scripts/apply-provenance.sh` — exiftool batch script that applies IPTC `DigitalSourceType` (`TrainedAlgorithmicMediaDigitalSource`) and CC0 XMP metadata (`Rights`, `License`, `AIIdentifier`) to all 186 WebP images. Idempotent (skips already-tagged files via `-if 'not $DigitalSourceType'` guard). Supports `--dry` dry-run mode.
  - **Z2.5 — AI transparency manifest**: `/.well-known/ai-transparency.json` — per-route AI content scope manifest for all 22 site routes. Declares text/image/code/styles AI provenance with tool attribution (Claude/GPT-4, EvoLink GPT Image 2), editorial review status, and CC0 licensing.
...
- **Homepage overhaul (G1)**: SEO foundation (description, h1, Organization JSON-LD), hero with CTA button, stat bridge trust signal, article feed grid showing up to 6 featured articles. New include: `_includes/article-feed.html`.
- **Om-oss improvements (G2)**: New "Slik jobber vi" section linking to `/metode/`. Added PostalAddress to Organization JSON-LD (Oslo, NO).
...
- **Always-loaded publicability rule (T9)**: Codified the may/must-not-commit split as a standing directive in `.opencode/opencode.json` (always in context) with detail in `.omo/rules/publicability.md` — current implementation and normative spec/design may be committed to this public repo; temporary reasoning, prompt seeds, outdated plans, and other non-public working material must not. Sensitive values must never enter commit messages, CHANGELOG entries, comments, fixtures, generated files, CI output, or issue/PR templates.
- **Sensitivity scanner (T10)**: Added a deterministic, non-LLM pre-commit gate `scripts/scan-sensitivity.mjs` — scans the full tree (`npm run scan:sensitivity`) or only the branch diff vs `origin/main` (`npm run scan:sensitivity:changed`, fallback `HEAD~1`) and flags content unfit for this public repo: percentage-based fee language, non-disclosure wording, internal gap assessments, pipeline/prospect material, raw contact lists, forecast wording, credential-shaped strings, and temporary filenames. Output is strictly `ruleId|path|line|reason` — matched values are never echoed — so it is safe to run in CI. Exit codes 0 = clean, 1 = hit, 2 = usage. Patterns in `scripts/sensitivity-rules.json`; verified false positives in `scripts/sensitivity-allowlist.json`; CI wrapper `scripts/ci-scan-sensitivity.sh`. Covered by `tests/scan-sensitivity.test.js` with synthetic fixtures under `tests/fixtures/sensitivity/`.
- **Gitleaks secret scanning (T11)**: Pinned Gitleaks v8.30.1 (release binary, checksum-verified) with a repo config `.gitleaks.toml` that extends the default rule set. Two scan modes via `npm run scan:secrets` (changed content, base `origin/main` → fallback `HEAD~1` → full tree) and `npm run scan:secrets:history` (full git history, `fetch-depth: 0` in CI). All output `--redact`ed. Allowlist is commit-scoped to a single verified-FP commit (synthetic test strings only) plus gitignored build dirs — no real credential values are suppressed. Smoke test `npm run test:secrets` validates install, clean changed scan, and redaction. Pinned version and scan commands documented in `.omo/rules/linting.md`.
...
### Added
- **Visual quality improvements batch**: Article reading-width wrapper (`_layouts/page.html`) — conditionally wraps content in `.section.container--wide.article-body` when `page.hero` exists. Section vertical rhythm in `article.css` (`--space-4xl`/`--space-2xl`/`--space-lg` spacers for h2/h3/p/ul/ol/img). Scroll-triggered animations on article body wrapper + references section. Google Fonts Roboto loaded via `<link>` with `font-display: swap`. Heading letter-spacing (h1: `-0.02em`, h2/h3: `-0.01em`) and `-webkit-font-smoothing: antialiased` on body. Blockquote styling with azure left border. Homepage welcome content in `index.md` with empty-section guards in `_layouts/home.html`. Smooth scrolling (`scroll-behavior: smooth`) on `html`. `:focus-visible` keyboard focus indicators with azure outline. Fixed back-to-top button in `_layouts/default.html`. Hero section now has `animate-on-scroll fade-in-up` classes.
