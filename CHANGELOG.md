...
- **Image provenance script (Z2.4)**: Created `scripts/apply-provenance.sh` — exiftool batch script applying IPTC `DigitalSourceType` and CC0 XMP metadata to all 186 WebP images. Idempotent via `-if 'not $DigitalSourceType'` guard. Supports `--dry` dry-run mode.
- **AI transparency manifest (Z2.5)**: Created `/.well-known/ai-transparency.json` — per-route AI content scope manifest for all 22 site routes. Declares text/image/code/styles AI provenance with tool attribution (Claude/GPT-4, EvoLink GPT Image 2), editorial review status, and CC0 licensing.
