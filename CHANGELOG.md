...
## [Unreleased]
- **CSS structural selector convergence**: Replaced explicit kramdown IAL class annotations with `:has()` structural selectors for three visual patterns: `.info-card-list` → `ul:has(> li > h3)`, `.challenge-grid` → `ul:has(> li > h4)`, `.section-illustration` → `:not(li) > p:has(> img:only-child) > img:not([class])`. Removed dead CSS (`.challenge-card`, `.grid.grid--2.challenges-grid`, float variants). Cleaned all annotations from 17 `_pages/*.md` files — 92 occurrences removed. CSS layer unchanged: `_includes/`, `_layouts/`, and `_data/` untouched.
