...
### Added
- **FF1 — Tag lookup page `/emne/`**: New `_tags` collection with 12 tag pages (tillit, psykologisk trygghet, servant leadership, kunstig intelligens, digital transformasjon, ledelse, usikkerhet, endringsledelse, organisasjonskultur, forankring, beslutningstaking, makt). `_layouts/tag.html` filters articles by tag. `_pages/emne.md` landing page shows tag cloud with article counts. Tags added as frontmatter to 5 benefit articles. CSS for `.tag-cloud`, `.tag-badge`, and dark mode variants in `article.css`.
